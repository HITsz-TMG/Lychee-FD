"""
Common reusable data-processing utilities.
"""
...
def tokenizer_image_token(prompt, tokenizer, image_token, image_token_index, add_special_tokens=True) -> List:
    ...
    input_ids = []
    offset = 0
    # If the tokenizer adds BOS by default, use offset=1.
...
def preprocess_pretraining(
    ...
    """
    Pretraining preprocessing that builds next-token labels for text.

    Compared with calling tokenizer.__call__ directly, this helper supports
    image_token values that are not in the tokenizer vocabulary. It does not
    support image_start or image_end tokens that are missing from the vocabulary.
    Args:
        sentence:
        tokenizer:
        image_token:
...
def preprocess_supervised(
    ...
    for i, message in enumerate(sentence):
        ...
        if i == 0 and adding_sys_in_query:
            value = system_message + value  # Llama-style system prompts are inserted into the round message.
        if i % 2 == 0:
