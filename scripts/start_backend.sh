...
MODE="${1:-stable}"
BACKEND_RAW="${2:-${STEPAUDIO_BACKEND:-vllm}}"
...
VLLM_SOURCE_DIR="${STEPAUDIO_VLLM_SOURCE_DIR:-${ROOT_DIR}/third_party/vllm}"
SYNC_FLASH_ATTN="${STEPAUDIO_VLLM_SYNC_FLASH_ATTN:-1}"
FORCE_SYNC_FLASH_ATTN="${STEPAUDIO_VLLM_FORCE_SYNC_FLASH_ATTN:-0}"

CONDA_ENV_PATH="${STEPAUDIO_CONDA_ENV_PATH:-}"
SERVER_NAME="${STEPAUDIO_SERVER_NAME:-0.0.0.0}"
SERVER_PORT="${STEPAUDIO_SERVER_PORT:-7860}"
FORCE_DISABLE_PROXY="${STEPAUDIO_FORCE_DISABLE_PROXY:-1}"
if [[ -z "${STEPAUDIO_MODEL_PATH:-}" ]]; then
  echo "[backend][ERROR] STEPAUDIO_MODEL_PATH is required." >&2
  echo "[backend][ERROR] Example: STEPAUDIO_MODEL_PATH=/path/to/checkpoint ./scripts/start_backend.sh stable vllm" >&2
  exit 1
fi
MODEL_PATH="${STEPAUDIO_MODEL_PATH}"
...
RUNTIME_LOG_ROOT="${STEPAUDIO_RUNTIME_LOG_ROOT:-${ROOT_DIR}/runtime_logs}"
RUNTIME_LOG_RUN_TAG="${STEPAUDIO_RUNTIME_LOG_RUN_TAG:-$(date '+%Y%m%d_%H%M%S')}"
RUNTIME_LOG_DIR="${STEPAUDIO_RUNTIME_LOG_DIR:-${RUNTIME_LOG_ROOT}/${RUNTIME_LOG_RUN_TAG}}"
mkdir -p "${RUNTIME_LOG_DIR}"

if is_truthy "${STEPAUDIO_REALTIME_DEBUG:-0}"; then
...
if [[ "${REALTIME_DEBUG_ENABLED}" == "1" ]]; then
  ...
  REALTIME_STAGE_TIMING_LOG_EFFECTIVE="${STEPAUDIO_REALTIME_STAGE_TIMING_LOG:-0}"
  VLLM_TOKEN_TRACE_EFFECTIVE="${STEPAUDIO_VLLM_TOKEN_TRACE:-0}"
fi

REALTIME_STAGE_TIMING_LOG_DIR_EFFECTIVE="${STEPAUDIO_REALTIME_STAGE_TIMING_LOG_DIR:-${RUNTIME_LOG_DIR}/realtime_stage_timing}"
REALTIME_CONTROL_PROB_TRACE_LOG_EFFECTIVE="${STEPAUDIO_REALTIME_CONTROL_PROB_TRACE_LOG:-0}"
REALTIME_CONTROL_PROB_TRACE_LOG_DIR_EFFECTIVE="${STEPAUDIO_REALTIME_CONTROL_PROB_TRACE_LOG_DIR:-${RUNTIME_LOG_DIR}/realtime_control_prob}"
VLLM_TOKEN_TRACE_PATH_EFFECTIVE="${STEPAUDIO_VLLM_TOKEN_TRACE_PATH:-${RUNTIME_LOG_DIR}/vllm_token_trace_${RUNTIME_LOG_RUN_TAG}.jsonl}"
STARTUP_CONFIG_LOG_PATH="${STEPAUDIO_STARTUP_CONFIG_LOG_PATH:-${RUNTIME_LOG_DIR}/startup_config.log}"
...
case "${MODE}" in
  stable)
    SIDE_PARALLEL_BRANCH="${STEPAUDIO_VLLM_SIDE_PARALLEL_BRANCH:-0}"
    GPU_MEM_UTIL="${STEPAUDIO_VLLM_GPU_MEMORY_UTILIZATION:-0.90}"
    ;;
  aggressive)
    SIDE_PARALLEL_BRANCH="${STEPAUDIO_VLLM_SIDE_PARALLEL_BRANCH:-1}"
    GPU_MEM_UTIL="${STEPAUDIO_VLLM_GPU_MEMORY_UTILIZATION:-0.92}"
    ;;
  *)
...
if [[ "${BACKEND_ARG_EXPLICIT}" == "1" && "${BACKEND}" == "hf" ]]; then
  ...
  EFFECTIVE_USE_VLLM="${STEPAUDIO_USE_VLLM:-${DEFAULT_USE_VLLM}}"
  REALTIME_INCREMENTAL_BACKEND="${STEPAUDIO_REALTIME_INCREMENTAL_BACKEND:-${DEFAULT_REALTIME_INCREMENTAL_BACKEND}}"
fi
...
echo "[backend] backend=${BACKEND} (STEPAUDIO_USE_VLLM=${EFFECTIVE_USE_VLLM})"
...
echo "[backend] startup_token2wav_first=${STEPAUDIO_STARTUP_TOKEN2WAV_FIRST:-1}"
echo "[backend] startup_warmup=${STEPAUDIO_STARTUP_WARMUP:-1}"
echo "[backend] startup_warmup_prompt_voice=${STEPAUDIO_STARTUP_WARMUP_PROMPT_VOICE:-male}"
echo "[backend] startup_warmup_token=${STEPAUDIO_STARTUP_WARMUP_TOKEN:-100}"
...
echo "[backend] vllm_ignore_text_eos=${STEPAUDIO_VLLM_IGNORE_TEXT_EOS:-1}"
...
if [[ "${EFFECTIVE_USE_VLLM}" == "1" ]]; then
  ...
  if [[ -d "${VLLM_SOURCE_DIR}" && "${RESOLVED_VLLM}" != "${VLLM_SOURCE_DIR}"* ]]; then
    echo "[backend][WARN] resolved vllm is not from STEPAUDIO_VLLM_SOURCE_DIR" >&2
  fi
fi
...
exec env \
  STEPAUDIO_REALTIME_DEBUG="${REALTIME_DEBUG_ENABLED}" \
  STEPAUDIO_REALTIME_STAGE_TIMING_LOG="${REALTIME_STAGE_TIMING_LOG_EFFECTIVE}" \
  STEPAUDIO_REALTIME_STAGE_TIMING_LOG_DIR="${REALTIME_STAGE_TIMING_LOG_DIR_EFFECTIVE}" \
  STEPAUDIO_REALTIME_CONTROL_PROB_TRACE_LOG="${REALTIME_CONTROL_PROB_TRACE_LOG_EFFECTIVE}" \
  ...
  STEPAUDIO_STOKEN_DELAY_NUM="${STEPAUDIO_STOKEN_DELAY_NUM:-20}" \
  STEPAUDIO_STOKEN_NO_REPEAT_NGRAM="${STEPAUDIO_STOKEN_NO_REPEAT_NGRAM:-4}" \
  STEPAUDIO_REALTIME_TTS_CHUNK_SIZE="${STEPAUDIO_REALTIME_TTS_CHUNK_SIZE:-1}" \
  STEPAUDIO_TTS_VOCODER_HOP_SIZE="${STEPAUDIO_TTS_VOCODER_HOP_SIZE:-25}" \
  STEPAUDIO_T2W_STREAM_LOOKAHEAD_LEN="${STEPAUDIO_T2W_STREAM_LOOKAHEAD_LEN:-3}" \
  STEPAUDIO_PROFILE_LATENCY="${STEPAUDIO_PROFILE_LATENCY:-0}" \
  STEPAUDIO_PROFILE_SIDE="${STEPAUDIO_PROFILE_SIDE:-0}" \
  STEPAUDIO_LOGITS_SANITIZE="${STEPAUDIO_LOGITS_SANITIZE:-0}" \
  STEPAUDIO_REALTIME_TRUE_INCREMENTAL_AUDIO="1" \
  STEPAUDIO_STARTUP_TOKEN2WAV_FIRST="${STEPAUDIO_STARTUP_TOKEN2WAV_FIRST:-1}" \
  STEPAUDIO_STARTUP_WARMUP="${STEPAUDIO_STARTUP_WARMUP:-1}" \
  STEPAUDIO_STARTUP_WARMUP_PROMPT_VOICE="${STEPAUDIO_STARTUP_WARMUP_PROMPT_VOICE:-male}" \
  STEPAUDIO_STARTUP_WARMUP_TOKEN="${STEPAUDIO_STARTUP_WARMUP_TOKEN:-100}" \
  STEPAUDIO_USE_VLLM="${EFFECTIVE_USE_VLLM}" \
  STEPAUDIO_REALTIME_INCREMENTAL_BACKEND="${REALTIME_INCREMENTAL_BACKEND}" \
  STEPAUDIO_VLLM_ENFORCE_EAGER="${STEPAUDIO_VLLM_ENFORCE_EAGER:-1}" \
  STEPAUDIO_VLLM_ENABLE_CHUNKED_PREFILL="${STEPAUDIO_VLLM_ENABLE_CHUNKED_PREFILL:-1}" \
  STEPAUDIO_VLLM_ENABLE_PREFIX_CACHING="${STEPAUDIO_VLLM_ENABLE_PREFIX_CACHING:-0}" \
  STEPAUDIO_VLLM_KEEP_ALIVE_SPEAKING="${STEPAUDIO_VLLM_KEEP_ALIVE_SPEAKING:-1}" \
  STEPAUDIO_VLLM_IGNORE_TEXT_EOS="${STEPAUDIO_VLLM_IGNORE_TEXT_EOS:-1}" \
  STEPAUDIO_VLLM_SIDE_SUBSET_PROJ="${STEPAUDIO_VLLM_SIDE_SUBSET_PROJ:-1}" \
  STEPAUDIO_VLLM_SIDE_SUBSET_REQUIRE_PROCESSOR="${STEPAUDIO_VLLM_SIDE_SUBSET_REQUIRE_PROCESSOR:-1}" \
  STEPAUDIO_VLLM_SIDE_PARALLEL_BRANCH="${SIDE_PARALLEL_BRANCH}" \
  STEPAUDIO_VLLM_TENSOR_PARALLEL_SIZE="${STEPAUDIO_VLLM_TENSOR_PARALLEL_SIZE:-1}" \
  STEPAUDIO_VLLM_PIPELINE_PARALLEL_SIZE="${STEPAUDIO_VLLM_PIPELINE_PARALLEL_SIZE:-1}" \
  STEPAUDIO_VLLM_GPU_MEMORY_UTILIZATION="${GPU_MEM_UTIL}" \
  STEPAUDIO_VLLM_MAX_MODEL_LEN="${STEPAUDIO_VLLM_MAX_MODEL_LEN:-4096}" \
  STEPAUDIO_VLLM_MAX_NUM_SEQS="${STEPAUDIO_VLLM_MAX_NUM_SEQS:-1}" \
  STEPAUDIO_VLLM_MAX_NUM_BATCHED_TOKENS="${STEPAUDIO_VLLM_MAX_NUM_BATCHED_TOKENS:-1024}" \
