...
def _import_websockets_module():
    """按需导入 SoulX sidecar 的 websocket 依赖。

    Import the optional websocket dependency used by the SoulX sidecar.
    """
    import websockets
...
SOULX_DUPLUG_ENABLED = str(
    os.environ.get("STEPAUDIO_SOULX_ENABLED", "0")
).strip().lower() in {"1", "true", "yes", "on"}
SOULX_DUPLUG_WS_URL = os.environ.get(
    "STEPAUDIO_SOULX_WS_URL", "ws://127.0.0.1:18080/turn"
).strip()
try:
    SOULX_DUPLUG_CLIENT_QUEUE_MAX = max(
        1, int(os.environ.get("STEPAUDIO_SOULX_CLIENT_QUEUE_MAX", "8"))
    )
except ValueError:
    SOULX_DUPLUG_CLIENT_QUEUE_MAX = 8
try:
    SOULX_DUPLUG_CONNECT_TIMEOUT_SEC = max(
        0.1, float(os.environ.get("STEPAUDIO_SOULX_CONNECT_TIMEOUT_SEC", "3.0"))
    )
except ValueError:
    SOULX_DUPLUG_CONNECT_TIMEOUT_SEC = 3.0
try:
    SOULX_DUPLUG_RECV_TIMEOUT_SEC = max(
        0.1, float(os.environ.get("STEPAUDIO_SOULX_RECV_TIMEOUT_SEC", "3.0"))
    )
except ValueError:
    SOULX_DUPLUG_RECV_TIMEOUT_SEC = 3.0
SOULX_DUPLUG_IDLE_COMPACT_ENABLED = str(
    os.environ.get("STEPAUDIO_SOULX_IDLE_COMPACT_ENABLED", "0")
).strip().lower() in {"1", "true", "yes", "on"}
SOULX_DUPLUG_IDLE_STATES = {
    item.strip().lower()
    for item in os.environ.get("STEPAUDIO_SOULX_IDLE_STATES", "idle,blank").split(",")
    if item.strip()
}
if not SOULX_DUPLUG_IDLE_STATES:
    SOULX_DUPLUG_IDLE_STATES = {"idle", "blank"}
SOULX_DUPLUG_ACTIVE_STATES = {
    item.strip().lower()
    for item in os.environ.get("STEPAUDIO_SOULX_ACTIVE_STATES", "nonidle,speak").split(",")
    if item.strip()
}
if not SOULX_DUPLUG_ACTIVE_STATES:
    SOULX_DUPLUG_ACTIVE_STATES = {"nonidle", "speak"}
try:
    SOULX_DUPLUG_IDLE_CONFIRM_MS = max(
        160, int(os.environ.get("STEPAUDIO_SOULX_IDLE_CONFIRM_MS", "2400"))
    )
except ValueError:
    SOULX_DUPLUG_IDLE_CONFIRM_MS = 2400
try:
    SOULX_DUPLUG_IDLE_MAX_AGE_MS = max(
        160, int(os.environ.get("STEPAUDIO_SOULX_IDLE_MAX_AGE_MS", "800"))
    )
except ValueError:
    SOULX_DUPLUG_IDLE_MAX_AGE_MS = 800
try:
    SOULX_DUPLUG_POST_ACTIVE_COOLDOWN_MS = max(
        0, int(os.environ.get("STEPAUDIO_SOULX_POST_ACTIVE_COOLDOWN_MS", "1200"))
    )
except ValueError:
    SOULX_DUPLUG_POST_ACTIVE_COOLDOWN_MS = 1200
try:
    SOULX_DUPLUG_IDLE_ANCHOR_MAX_PENDING_MS = max(
        0, int(os.environ.get("STEPAUDIO_SOULX_IDLE_ANCHOR_MAX_PENDING_MS", "400"))
    )
except ValueError:
    SOULX_DUPLUG_IDLE_ANCHOR_MAX_PENDING_MS = 400
try:
    SOULX_DUPLUG_IDLE_COMPACT_MIN_DROP_TOKENS = max(
        1, int(os.environ.get("STEPAUDIO_SOULX_IDLE_COMPACT_MIN_DROP_TOKENS", "1"))
    )
except ValueError:
    SOULX_DUPLUG_IDLE_COMPACT_MIN_DROP_TOKENS = 1
...
class SoulXDuplugClient:
    """SoulX-Duplug sidecar 客户端，用于旁路监听用户语音状态。

    SoulX-Duplug sidecar client used to observe user speech state out of band.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self._queue: "queue.Queue[dict]" = queue.Queue(maxsize=max(1, int(max_queue)))
    ...
    def enqueue(
        ...
        *,
        ...
        """把一段用户音频送入 SoulX 旁路队列。

        Enqueue one user-audio chunk for SoulX sidecar observation.
        """
        if not SOULX_DUPLUG_ENABLED or not self.ws_url:
            return
        if audio is None:
            return
        ...
        item = {
            ...
            "audio_start_sample": int(audio_start_sample) if audio_start_sample is not None else None,
            "audio_end_sample": int(audio_end_sample) if audio_end_sample is not None else None,
        }
        try:
            ...
            try:
                ...
                self._mark_error(str(session_id), "queue_full")

    def _worker_loop(self) -> None:
        try:
            asyncio.run(self._async_worker())
        except Exception as exc:
            logger.warning("SoulX-Duplug client stopped: %s", exc)

    async def _async_worker(self) -> None:
        try:
            websockets = _import_websockets_module()
        except Exception as exc:
            logger.warning("SoulX-Duplug disabled: failed to import websockets: %s", exc)
            return
        ...
        while not self._stop_event.is_set():
            ...
            try:
                async with websockets.connect(
                    ...
                    await self._send_item(ws, item)
                    while not self._stop_event.is_set():
                        try:
                            next_item = self._queue.get_nowait()
                        except queue.Empty:
                            next_item = await asyncio.to_thread(self._queue.get)
                        if isinstance(next_item, dict):
                            await self._send_item(ws, next_item)
            except Exception as exc:
                sid = str(item.get("session_id") or "")
                self._mark_error(sid, str(exc))
                logger.warning("SoulX-Duplug websocket error: %s", exc)
                await asyncio.sleep(0.5)

    async def _send_item(self, ws, item: dict) -> None:
        session_id = str(item.get("session_id") or "")
        audio = np.asarray(item.get("audio"), dtype=np.float32).reshape(-1)
        ...
        msg = json.loads(raw)
        state_payload = msg.get("state") if isinstance(msg, dict) else None
        state = None
        ...
        self._mark_state(
            ...
            seq=seq,
            audio_start_sample=item.get("audio_start_sample"),
            audio_end_sample=item.get("audio_end_sample"),
        )
    ...
    def _mark_state(
        ...
        *,
        ...
        payload: dict,
        seq: int,
        audio_start_sample: Optional[int],
        audio_end_sample: Optional[int],
    ) -> None:
        if not session_id:
            return
        session = _get_realtime_session(session_id)
        if session is None:
            return
        ...
        is_active = bool(
            state_norm
            and (
                state_norm in SOULX_DUPLUG_ACTIVE_STATES
                or state_norm not in SOULX_DUPLUG_IDLE_STATES
            )
        )
        try:
            audio_start = int(audio_start_sample) if audio_start_sample is not None else None
        except (TypeError, ValueError):
            audio_start = None
        try:
            audio_end = int(audio_end_sample) if audio_end_sample is not None else None
        except (TypeError, ValueError):
            audio_end = None
        with session.lock:
            session.soulx_state = state
            session.soulx_state_payload = dict(payload) if isinstance(payload, dict) else {}
            session.soulx_state_epoch_ms = now_ms
            session.soulx_seq = int(seq)
            session.soulx_error = None
            session.soulx_state_audio_start_sample = audio_start
            session.soulx_state_audio_end_sample = audio_end
            if is_active:
                session.soulx_last_active_state = state_norm
                session.soulx_last_active_epoch_ms = now_ms
                session.soulx_last_active_seq = int(seq)
                if audio_end is not None:
                    session.soulx_last_active_audio_end_sample = max(
                        int(session.soulx_last_active_audio_end_sample),
                        int(audio_end),
                    )
                session.soulx_idle_coverage_start_sample = None
                session.soulx_idle_coverage_end_sample = 0
                _reset_soulx_idle_anchor_unlocked(session, f"soulx_active:{state_norm}")
            elif state_norm in SOULX_DUPLUG_IDLE_STATES and audio_start is not None and audio_end is not None:
                idle_start = int(audio_start)
                idle_end = int(audio_end)
                if idle_end > idle_start:
                    last_active_end = int(session.soulx_last_active_audio_end_sample)
                    if last_active_end > idle_start:
                        idle_start = min(int(idle_end), int(last_active_end))
                    if idle_end > idle_start:
                        coverage_start = session.soulx_idle_coverage_start_sample
                        coverage_end = int(session.soulx_idle_coverage_end_sample)
                        if coverage_start is not None and idle_start <= coverage_end:
                            session.soulx_idle_coverage_end_sample = max(coverage_end, idle_end)
                        else:
                            session.soulx_idle_coverage_start_sample = int(idle_start)
                            session.soulx_idle_coverage_end_sample = int(idle_end)

    def _mark_error(self, session_id: str, error: str) -> None:
        if not session_id:
            return
        session = _get_realtime_session(session_id)
        if session is None:
            return
        with session.lock:
            session.soulx_error = str(error)


_soulx_duplug_client: Optional[SoulXDuplugClient] = None
_soulx_duplug_client_lock = threading.Lock()


def get_soulx_duplug_client() -> Optional[SoulXDuplugClient]:
    """获取 SoulX-Duplug 客户端；未启用时返回 None。

    Return the SoulX-Duplug client, or None when the sidecar is disabled.
    """
    global _soulx_duplug_client
    if not SOULX_DUPLUG_ENABLED:
        return None
    with _soulx_duplug_client_lock:
        if _soulx_duplug_client is None:
            _soulx_duplug_client = SoulXDuplugClient(
                ws_url=SOULX_DUPLUG_WS_URL,
                max_queue=SOULX_DUPLUG_CLIENT_QUEUE_MAX,
                connect_timeout_sec=SOULX_DUPLUG_CONNECT_TIMEOUT_SEC,
                recv_timeout_sec=SOULX_DUPLUG_RECV_TIMEOUT_SEC,
            )
            _soulx_duplug_client.start()
            logger.info(
                "SoulX-Duplug observe client enabled: url=%s queue_max=%d",
                SOULX_DUPLUG_WS_URL,
                SOULX_DUPLUG_CLIENT_QUEUE_MAX,
            )
        return _soulx_duplug_client
...
class RealtimeSessionState:
    ...
    soulx_state: Optional[str] = None
    soulx_state_payload: dict = field(default_factory=dict)
    soulx_state_epoch_ms: Optional[int] = None
    soulx_state_audio_start_sample: Optional[int] = None
    soulx_state_audio_end_sample: Optional[int] = None
    soulx_seq: int = 0
    soulx_error: Optional[str] = None
    soulx_last_active_state: Optional[str] = None
    soulx_last_active_epoch_ms: Optional[int] = None
    soulx_last_active_audio_end_sample: int = 0
    soulx_last_active_seq: int = 0
    soulx_idle_coverage_start_sample: Optional[int] = None
    soulx_idle_coverage_end_sample: int = 0
    soulx_idle_candidate_since_epoch_ms: Optional[int] = None
    soulx_idle_anchor_prefix: Optional[dict] = None
    soulx_idle_anchor_state: str = "l"
    soulx_idle_anchor_total_received_samples: int = 0
    soulx_idle_anchor_processed_samples: int = 0
    soulx_idle_compact_count: int = 0
    soulx_idle_last_compact_epoch_ms: Optional[int] = None
    soulx_idle_last_reset_reason: Optional[str] = None
...
def _build_soulx_observation_unlocked(session: RealtimeSessionState) -> dict:
    now_ms = int(time.time() * 1000)
    state_epoch_ms = session.soulx_state_epoch_ms
    age_ms = None
    if state_epoch_ms is not None:
        try:
            age_ms = max(0, now_ms - int(state_epoch_ms))
        except (TypeError, ValueError):
            age_ms = None
    payload = session.soulx_state_payload if isinstance(session.soulx_state_payload, dict) else {}
    return {
        "enabled": bool(SOULX_DUPLUG_ENABLED),
        "url": SOULX_DUPLUG_WS_URL if SOULX_DUPLUG_ENABLED else None,
        "state": session.soulx_state,
        "seq": int(session.soulx_seq),
        "state_epoch_ms": int(state_epoch_ms) if state_epoch_ms is not None else None,
        "age_ms": int(age_ms) if age_ms is not None else None,
        "state_audio_start_sample": (
            int(session.soulx_state_audio_start_sample)
            if session.soulx_state_audio_start_sample is not None
            else None
        ),
        "state_audio_end_sample": (
            int(session.soulx_state_audio_end_sample)
            if session.soulx_state_audio_end_sample is not None
            else None
        ),
        "last_active_state": session.soulx_last_active_state,
        "last_active_epoch_ms": (
            int(session.soulx_last_active_epoch_ms)
            if session.soulx_last_active_epoch_ms is not None
            else None
        ),
        "last_active_audio_end_sample": int(session.soulx_last_active_audio_end_sample),
        "last_active_seq": int(session.soulx_last_active_seq),
        "idle_coverage_start_sample": (
            int(session.soulx_idle_coverage_start_sample)
            if session.soulx_idle_coverage_start_sample is not None
            else None
        ),
        "idle_coverage_end_sample": int(session.soulx_idle_coverage_end_sample),
        "error": session.soulx_error,
        "payload": dict(payload),
    }


def _copy_realtime_prefix(prefix: Optional[dict]) -> Optional[dict]:
    if not isinstance(prefix, dict):
        return None
    copied = {}
    for key in ("input_ids", "stoken_ids", "control_input_ids"):
        value = prefix.get(key)
        if isinstance(value, (list, tuple)):
            copied[key] = [int(x) for x in value]
        else:
            copied[key] = []
    return copied


def _realtime_prefix_len(prefix: Optional[dict]) -> int:
    if not isinstance(prefix, dict):
        return 0
    value = prefix.get("input_ids")
    if isinstance(value, (list, tuple)):
        return int(len(value))
    return 0


def _reset_soulx_idle_anchor_unlocked(session: RealtimeSessionState, reason: str) -> None:
    if session.soulx_idle_candidate_since_epoch_ms is not None:
        session.soulx_idle_last_reset_reason = str(reason)
    session.soulx_idle_candidate_since_epoch_ms = None
    session.soulx_idle_anchor_prefix = None
    session.soulx_idle_anchor_state = "l"
    session.soulx_idle_anchor_total_received_samples = 0
    session.soulx_idle_anchor_processed_samples = 0


def _maybe_compact_soulx_idle_unlocked(
    session: RealtimeSessionState,
    *,
    round_prefix: Optional[dict],
    round_state: str,
    snapshot_total_received: int,
    snapshot_pending_samples: int,
    soulx_observation: dict,
    tts_event_active: bool,
) -> Optional[dict]:
    if not (SOULX_DUPLUG_ENABLED and SOULX_DUPLUG_IDLE_COMPACT_ENABLED):
        return None

    now_ms = int(time.time() * 1000)
    soulx_state = str(soulx_observation.get("state") or "").strip().lower()
    soulx_age_ms = soulx_observation.get("age_ms")
    try:
        soulx_age_ms_value = int(soulx_age_ms) if soulx_age_ms is not None else None
    except (TypeError, ValueError):
        soulx_age_ms_value = None
    soulx_error = soulx_observation.get("error")
    processed_samples = max(0, int(snapshot_total_received) - int(snapshot_pending_samples))
    confirm_samples = int(
        math.ceil(float(INPUT_SAMPLE_RATE) * float(SOULX_DUPLUG_IDLE_CONFIRM_MS) / 1000.0)
    )
    last_active_audio_end_sample = int(soulx_observation.get("last_active_audio_end_sample") or 0)
    try:
        idle_coverage_start_sample = (
            int(soulx_observation.get("idle_coverage_start_sample"))
            if soulx_observation.get("idle_coverage_start_sample") is not None
            else None
        )
    except (TypeError, ValueError):
        idle_coverage_start_sample = None
    try:
        idle_coverage_end_sample = int(soulx_observation.get("idle_coverage_end_sample") or 0)
    except (TypeError, ValueError):
        idle_coverage_end_sample = 0
    idle_coverage_complete = (
        idle_coverage_start_sample is not None
        and int(idle_coverage_start_sample) <= int(processed_samples)
        and int(idle_coverage_end_sample) >= int(processed_samples)
    )
    covered_processed_sample = (
        min(int(processed_samples), int(idle_coverage_end_sample))
        if idle_coverage_start_sample is not None
        else 0
    )

    if round_state != "l":
        _reset_soulx_idle_anchor_unlocked(session, "main_not_listening")
        return None
    if tts_event_active:
        _reset_soulx_idle_anchor_unlocked(session, "tts_event_active")
        return None
    if soulx_error:
        _reset_soulx_idle_anchor_unlocked(session, "soulx_error")
        return None
    if soulx_state not in SOULX_DUPLUG_IDLE_STATES:
        _reset_soulx_idle_anchor_unlocked(session, "soulx_not_idle")
        return None
    if not idle_coverage_complete:
        return None

    if session.soulx_idle_candidate_since_epoch_ms is None:
        if _realtime_prefix_len(round_prefix) <= 0:
            return None
        session.soulx_idle_candidate_since_epoch_ms = now_ms
        session.soulx_idle_anchor_prefix = _copy_realtime_prefix(round_prefix)
        session.soulx_idle_anchor_state = "l"
        session.soulx_idle_anchor_total_received_samples = int(snapshot_total_received)
        session.soulx_idle_anchor_processed_samples = int(processed_samples)
        session.soulx_idle_last_reset_reason = None
        return None

    if (
        idle_coverage_start_sample is not None
        and int(session.soulx_idle_anchor_processed_samples) < int(idle_coverage_start_sample)
    ):
        _reset_soulx_idle_anchor_unlocked(session, "anchor_before_idle_coverage")
        return None

    idle_samples = max(
        0,
        int(covered_processed_sample) - int(session.soulx_idle_anchor_processed_samples),
    )
    idle_ms = int(round(float(idle_samples) * 1000.0 / float(INPUT_SAMPLE_RATE)))
    if int(idle_samples) < int(confirm_samples):
        return None

    anchor_prefix = _copy_realtime_prefix(session.soulx_idle_anchor_prefix)
    before_prefix_len = _realtime_prefix_len(round_prefix)
    anchor_prefix_len = _realtime_prefix_len(anchor_prefix)
    dropped_prefix_tokens = max(0, int(before_prefix_len) - int(anchor_prefix_len))
    audio_cache_snapshot = None
    audio_cache_token_count = 0
    audio_cache_window_count = 0
    event = {
        "enabled": True,
        "reason": "soulx_idle_confirmed",
        "mode": "rebuild",
        "kv_truncate_attempted": False,
        "kv_truncate_ok": False,
        "kv_truncate_reason": None,
        "kv_freed_blocks": 0,
        "idle_ms": int(idle_ms),
        "idle_samples": int(idle_samples),
        "confirm_ms": int(SOULX_DUPLUG_IDLE_CONFIRM_MS),
        "confirm_samples": int(confirm_samples),
        "sync_mode": "sample_index",
        "soulx_state": soulx_state,
        "soulx_age_ms": int(soulx_age_ms_value) if soulx_age_ms_value is not None else None,
        "before_prefix_len": int(before_prefix_len),
        "anchor_prefix_len": int(anchor_prefix_len),
        "dropped_prefix_tokens": int(dropped_prefix_tokens),
        "min_drop_tokens": int(SOULX_DUPLUG_IDLE_COMPACT_MIN_DROP_TOKENS),
        "anchor_total_received_samples": int(session.soulx_idle_anchor_total_received_samples),
        "anchor_processed_samples": int(session.soulx_idle_anchor_processed_samples),
        "current_total_received_samples": int(snapshot_total_received),
        "current_processed_samples": int(processed_samples),
        "current_pending_samples": int(snapshot_pending_samples),
        "idle_coverage_start_sample": int(idle_coverage_start_sample),
        "idle_coverage_end_sample": int(idle_coverage_end_sample),
        "covered_processed_sample": int(covered_processed_sample),
        "last_active_state": soulx_observation.get("last_active_state"),
        "last_active_audio_end_sample": int(last_active_audio_end_sample),
    }

    if int(anchor_prefix_len) <= 0:
        session.soulx_idle_candidate_since_epoch_ms = now_ms
        session.soulx_idle_anchor_prefix = _copy_realtime_prefix(round_prefix)
        session.soulx_idle_anchor_state = "l"
        session.soulx_idle_anchor_total_received_samples = int(snapshot_total_received)
        session.soulx_idle_anchor_processed_samples = int(processed_samples)
        session.soulx_idle_last_reset_reason = "invalid_anchor_prefix"
        event["skipped"] = True
        event["reason"] = "invalid_anchor_prefix"
        event["compact_count"] = int(session.soulx_idle_compact_count)
        return event

    if int(dropped_prefix_tokens) < int(SOULX_DUPLUG_IDLE_COMPACT_MIN_DROP_TOKENS):
        session.soulx_idle_candidate_since_epoch_ms = now_ms
        session.soulx_idle_anchor_prefix = _copy_realtime_prefix(round_prefix)
        session.soulx_idle_anchor_state = "l"
        session.soulx_idle_anchor_total_received_samples = int(snapshot_total_received)
        session.soulx_idle_anchor_processed_samples = int(processed_samples)
        session.soulx_idle_last_reset_reason = "insufficient_prefix_drop"
        event["skipped"] = True
        event["reason"] = "insufficient_prefix_drop"
        event["compact_count"] = int(session.soulx_idle_compact_count)
        return event

    truncate_active_request = getattr(
        session.incremental_stream_session,
        "truncate_active_request_to_prefix",
        None,
    )
    if callable(truncate_active_request):
        event["kv_truncate_attempted"] = True
        try:
            truncate_result = truncate_active_request(anchor_prefix)
        except Exception as exc:
            truncate_result = {
                "ok": False,
                "reason": "truncate_exception",
                "error": str(exc),
            }
        if isinstance(truncate_result, dict):
            event["kv_truncate_ok"] = bool(truncate_result.get("ok"))
            event["kv_truncate_reason"] = str(truncate_result.get("reason") or "")
            event["kv_freed_blocks"] = int(truncate_result.get("freed_blocks") or 0)
            event["kv_kept_blocks"] = int(truncate_result.get("kept_blocks") or 0)
            event["kv_block_size"] = int(truncate_result.get("block_size") or 0)
            event["audio_cache_tokens"] = int(truncate_result.get("audio_cache_tokens") or 0)
            event["audio_cache_windows"] = int(truncate_result.get("audio_cache_windows") or 0)
        else:
            event["kv_truncate_reason"] = "truncate_returned_non_dict"
        if bool(event["kv_truncate_ok"]):
            session.realtime_stream_audio_cache_snapshot = None
            session.realtime_prefix = anchor_prefix
            session.realtime_listening_state = "l"
            session.soulx_idle_compact_count += 1
            session.soulx_idle_last_compact_epoch_ms = now_ms

            session.soulx_idle_candidate_since_epoch_ms = now_ms
            session.soulx_idle_anchor_prefix = _copy_realtime_prefix(session.realtime_prefix)
            session.soulx_idle_anchor_state = "l"
            session.soulx_idle_anchor_total_received_samples = int(snapshot_total_received)
            session.soulx_idle_anchor_processed_samples = int(processed_samples)
            session.soulx_idle_last_reset_reason = None
            event["mode"] = "vllm_tail_truncate"
            event["compact_count"] = int(session.soulx_idle_compact_count)
            return event

    if int(anchor_prefix_len) > 0:
        export_audio_cache = getattr(
            session.incremental_stream_session,
            "export_incremental_audio_cache",
            None,
        )
        if callable(export_audio_cache):
            try:
                audio_cache_snapshot = export_audio_cache(int(anchor_prefix_len))
            except Exception as exc:
                audio_cache_snapshot = None
                event["audio_cache_error"] = str(exc)
        if isinstance(audio_cache_snapshot, dict):
            try:
                audio_cache_token_count = int(
                    audio_cache_snapshot.get("audio_input_token_count") or 0
                )
            except (TypeError, ValueError):
                audio_cache_token_count = 0
            try:
                audio_cache_window_count = int(
                    audio_cache_snapshot.get("audio_window_count") or 0
                )
            except (TypeError, ValueError):
                audio_cache_window_count = 0
        event["audio_cache_tokens"] = int(audio_cache_token_count)
        event["audio_cache_windows"] = int(audio_cache_window_count)
        if int(audio_cache_token_count) < int(anchor_prefix_len):
            session.soulx_idle_candidate_since_epoch_ms = now_ms
            session.soulx_idle_anchor_prefix = _copy_realtime_prefix(round_prefix)
            session.soulx_idle_anchor_state = "l"
            session.soulx_idle_anchor_total_received_samples = int(snapshot_total_received)
            session.soulx_idle_anchor_processed_samples = int(processed_samples)
            session.soulx_idle_last_reset_reason = "audio_cache_unavailable"
            event["skipped"] = True
            event["reason"] = "audio_cache_unavailable"
            event["compact_count"] = int(session.soulx_idle_compact_count)
            return event

    session.realtime_stream_audio_cache_snapshot = audio_cache_snapshot
...
def _run_realtime_session_worker(session_id: str) -> None:
    ...
    while True:
        ...
        with session.lock:
            ...
            round_soulx_observation = _build_soulx_observation_unlocked(session)
            round_tts_bridge = session.tts_bridge
            round_tts_event_active = bool(round_tts_bridge.event_active) if round_tts_bridge is not None else False
            round_soulx_compaction = _maybe_compact_soulx_idle_unlocked(
                session,
                round_prefix=round_prefix,
                round_state=round_state,
                snapshot_total_received=snapshot_total_received,
                snapshot_pending_samples=snapshot_pending_samples,
                soulx_observation=round_soulx_observation,
                tts_event_active=round_tts_event_active,
            )
            if isinstance(round_soulx_compaction, dict):
                round_prefix = session.realtime_prefix
                round_state = (
                    session.realtime_listening_state
                    if session.realtime_listening_state in {"l", "s", "b"}
                    else "l"
                )
                round_stream_session = session.incremental_stream_session
            round_prefix_len = (
        ...
        logger.info(
            "Realtime round=%d start prefix_len=%d state=%s total_sec=%.2f pending_sec=%.2f consume_sec=%.2f audio_mode=%s soulx_state=%s soulx_age_ms=%s soulx_error=%s",
            round_id,
            round_prefix_len,
            round_state,
            ...
            round_soulx_observation.get("state"),
            round_soulx_observation.get("age_ms"),
            round_soulx_observation.get("error"),
        )
        if isinstance(round_soulx_compaction, dict):
            if bool(round_soulx_compaction.get("skipped")):
                logger.info(
                    "SoulX idle compaction skipped round=%d reason=%s idle_ms=%d prefix_len=%d->%d dropped=%d min_drop=%d audio_cache_tokens=%d compact_count=%d mode=%s sync=%s coverage=%d->%d processed=%d kv_reason=%s",
                    round_id,
                    str(round_soulx_compaction.get("reason") or "unknown"),
                    int(round_soulx_compaction.get("idle_ms", 0)),
                    int(round_soulx_compaction.get("before_prefix_len", 0)),
                    int(round_soulx_compaction.get("anchor_prefix_len", 0)),
                    int(round_soulx_compaction.get("dropped_prefix_tokens", 0)),
                    int(round_soulx_compaction.get("min_drop_tokens", 1)),
                    int(round_soulx_compaction.get("audio_cache_tokens", 0)),
                    int(round_soulx_compaction.get("compact_count", 0)),
                    str(round_soulx_compaction.get("mode") or "unknown"),
                    str(round_soulx_compaction.get("sync_mode") or "unknown"),
                    int(round_soulx_compaction.get("idle_coverage_start_sample", 0)),
                    int(round_soulx_compaction.get("idle_coverage_end_sample", 0)),
                    int(round_soulx_compaction.get("current_processed_samples", 0)),
                    str(round_soulx_compaction.get("kv_truncate_reason") or ""),
                )
            else:
                logger.info(
                    "SoulX idle compaction round=%d idle_ms=%d prefix_len=%d->%d dropped=%d audio_cache_tokens=%d audio_cache_windows=%d compact_count=%d mode=%s sync=%s coverage=%d->%d processed=%d kv_truncate=%s kv_freed_blocks=%d kv_kept_blocks=%d kv_block_size=%d kv_reason=%s",
                    round_id,
                    int(round_soulx_compaction.get("idle_ms", 0)),
                    int(round_soulx_compaction.get("before_prefix_len", 0)),
                    int(round_soulx_compaction.get("anchor_prefix_len", 0)),
                    int(round_soulx_compaction.get("dropped_prefix_tokens", 0)),
                    int(round_soulx_compaction.get("audio_cache_tokens", 0)),
                    int(round_soulx_compaction.get("audio_cache_windows", 0)),
                    int(round_soulx_compaction.get("compact_count", 0)),
                    str(round_soulx_compaction.get("mode") or "unknown"),
                    str(round_soulx_compaction.get("sync_mode") or "unknown"),
                    int(round_soulx_compaction.get("idle_coverage_start_sample", 0)),
                    int(round_soulx_compaction.get("idle_coverage_end_sample", 0)),
                    int(round_soulx_compaction.get("current_processed_samples", 0)),
                    bool(round_soulx_compaction.get("kv_truncate_ok")),
                    int(round_soulx_compaction.get("kv_freed_blocks", 0)),
                    int(round_soulx_compaction.get("kv_kept_blocks", 0)),
                    int(round_soulx_compaction.get("kv_block_size", 0)),
                    str(round_soulx_compaction.get("kv_truncate_reason") or ""),
                )
        ...
        try:
            ...
            with session.lock:
                ...
                if isinstance(round_trace, dict):
                    ...
                    round_trace["soulx_duplug"] = dict(round_soulx_observation)
                    if isinstance(round_soulx_compaction, dict):
                        round_trace["soulx_idle_compaction"] = dict(round_soulx_compaction)
...
def register_realtime_session_routes(app) -> None:
    ...
    async def push_realtime_chunk(session_id: str, request: Request):
        ...
        soulx_client = get_soulx_duplug_client()
        if soulx_client is not None:
            soulx_client.enqueue(
                session_id,
                chunk_np,
                audio_start_sample=chunk_start_sample,
                audio_end_sample=chunk_end_sample,
            )
