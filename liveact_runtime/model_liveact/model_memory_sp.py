...
from liveact_runtime.fp8_gemm import FP8Linear
import logging
...
#     USE_SAGEATTN = True
#     print("[LiveAct] Using SageAttention FP16", flush=True)
# except Exception as exc:
#     USE_SAGEATTN = False
#     print(f"[LiveAct] SageAttention FP16 unavailable: {exc}", flush=True)

try:
    from sageattention import sageattn
    USE_SAGEATTN = True
    print("[LiveAct] Using SageAttention FP8 SM90", flush=True)
except Exception as exc:
    USE_SAGEATTN = False
    print(f"[LiveAct] SageAttention unavailable: {exc}", flush=True)
