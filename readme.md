...
For **NPU (Ascend 910B)** users, additionally install:
```bash
...
┌─────────────────────────────────────────────────┐
│                  Input Embeddings                 │
│  text_embed + stoken_embed + control_embed       │
│  + audio_features (from AudioEncoder+Adaptor)    │
└──────────────────────┬──────────────────────────┘
                       │
              ┌────────▼────────┐
              │  Backbone LLM   │
              ...
              └───┬────┬────┬───┘
                  │    │    │
    hidden_states[L-s] │  hidden_states[L-c]
                  │    │    │
         ┌────────▼┐   │   ┌▼────────┐
         │ Stoken  │   │   │ Control │
         │ Branch  │   │   │ Branch  │
...
### Launch Training (NPU / Ascend 910B)

```bash
# Use train_npu.py instead, which includes torch_npu imports
...
1. **Text Loss**: Next-token prediction on text tokens (standard cross-entropy)
2. **Stoken Loss**: Next-token prediction on speech tokens (clamped to stoken vocabulary range)
3. **Control Loss**: Next-token prediction on control tokens (clamped to control vocabulary range)

Total loss = text_loss + stoken_loss + control_loss

## Citation

```bibtex
@inproceedings{anonymous2026fullduplex,
    title={Full-Duplex Speech Language Model with Multi-Stream Turn-Taking Control},
    author={Anonymous},
    year={2026}
}
```
